...
def process_fifo_issue(item_id, qty_required, conn):
    ...
    remaining_to_issue = qty_required
    total_cost = 0.0
    for batch_id, qty_rem, unit_price in batches:
        if remaining_to_issue <= 0:
            break
        take = min(qty_rem, remaining_to_issue)
        total_cost += take * unit_price
        c.execute("UPDATE fifo_batches SET qty_remaining = qty_remaining - ? WHERE batch_id = ?",
                  (take, batch_id))
        remaining_to_issue -= take
    avg_price = round(total_cost / qty_required, 4)
...
st.sidebar.markdown("---")
st.sidebar.caption("Factory Store Management System v1.0")
...
if menu == "🏠 Dashboard":
    ...
    with col2:
        ...
        recent_in = pd.read_sql(
            "SELECT bill_no, bill_date, supplier_name FROM inward_entries ORDER BY created_at DESC LIMIT 8",
            conn)
        conn.close()
    ...
    tab1, tab2 = st.tabs(["➕ Add / Edit Item", "📋 View All Items"])
    with tab1:
    ...
    with tab2:
        ...
        if df.empty:
            ...
            search = st.text_input("🔍 Search items", placeholder="Type item name, code, or category...")
            if search:
    ...
    tab1, tab2 = st.tabs(["➕ New Inward Entry", "📋 View Inward Records"])
    with tab1:
        ...
        if not item_opts:
            ...
            with st.form("add_inward_item", clear_on_submit=True):
                ...
                add_item = st.form_submit_button("➕ Add Item to List")
                if add_item:
        ...
        if st.session_state.inward_items:
            ...
            st.dataframe(df_items[['item_name','quantity','unit_price','gst_percent','gst_amount','total_value']],
                         use_container_width=True, hide_index=True)
            grand_total = df_items['total_value'].sum()
            st.markdown(f"**Grand Total: {format_inr(grand_total)}**")
            ccl, csub = st.columns([1, 1])
    ...
    with tab2:
        ...
        if df_all.empty:
            ...
            search_bill = col1.text_input("🔍 Search Bill No / Supplier")
            from_dt = col2.date_input("From Date", value=date.today() - timedelta(days=30), key="in_from")
            to_dt   = col3.date_input("To Date",   value=date.today(), key="in_to")
            df_all['bill_date'] = pd.to_datetime(df_all['bill_date'])
            df_filtered = df_all[(df_all['bill_date'] >= pd.Timestamp(from_dt)) &
                                  (df_all['bill_date'] <= pd.Timestamp(to_dt))]
            if search_bill:
                df_filtered = df_filtered[df_filtered.apply(lambda r: search_bill.lower() in str(r).lower(), axis=1)]
            st.dataframe(df_filtered[['bill_no','bill_date','supplier_name','security_entry_no','security_entry_date','remarks']],
                         use_container_width=True, hide_index=True)
            if not df_filtered.empty:
                sel_inward_id = st.selectbox("Select Entry to view details",
                                              df_filtered['inward_id'].tolist(),
                                              format_func=lambda x: df_filtered.set_index('inward_id').loc[x,'bill_no'])
                conn = get_conn()
                det = pd.read_sql(
                    "SELECT d.*, i.item_code, i.item_name, i.unit FROM inward_details d "
                    "JOIN items i ON d.item_id=i.item_id WHERE d.inward_id=?",
                    conn, params=(sel_inward_id,))
                conn.close()
                if not det.empty:
    ...
    tab1, tab2 = st.tabs(["➕ New Issue Entry", "📋 View Issue Records"])
    with tab1:
        ...
        if not item_opts:
            ...
            with st.form("add_outward_item", clear_on_submit=True):
                ...
                if not stock_df.empty and item_opts:
                    item_id_preview = item_opts.get(sel_item_out)
                    avail = stock_df[stock_df['item_id'] == item_id_preview]['qty_in_stock'].values
                    avail_qty = avail[0] if len(avail) > 0 else 0
                    avg_p = stock_df[stock_df['item_id'] == item_id_preview]['avg_price'].values
                    avg_price_preview = avg_p[0] if len(avg_p) > 0 else 0
                    st.caption(f"📦 Available Stock: **{avail_qty}** | Avg Price: **{format_inr(avg_price_preview)}**")
                add_out = st.form_submit_button("➕ Add Item to List")
                if add_out:
                    item_id_sel = item_opts[sel_item_out]
                    avail_row = stock_df[stock_df['item_id'] == item_id_sel]
                    avail_qty2 = avail_row['qty_in_stock'].values[0] if not avail_row.empty else 0
                    if qty_out > avail_qty2:
                        st.error(f"⚠️ Only {avail_qty2} in stock. Cannot issue {qty_out}.")
                    else:
                        st.session_state.outward_items.append({
                            "item_id": item_id_sel,
                            "item_name": sel_item_out,
                            "quantity": qty_out,
                        })
                        st.rerun()
        if st.session_state.outward_items:
            st.subheader("🧾 Items to Issue")
            df_out = pd.DataFrame(st.session_state.outward_items)
            st.dataframe(df_out[['item_name','quantity']], use_container_width=True, hide_index=True)
            ...
            if co2.button("✅ Submit Issue Entry", type="primary", use_container_width=True):
                if not all([indent_no, issued_to]):
                    ...
                    for row in st.session_state.outward_items:
                        ...
                        if err:
                            errors.append(f"{row['item_name']}: {err}")
                        else:
                            total_val = round(avg_price * row['quantity'], 2)
                            c.execute(
                                "INSERT INTO outward_details (outward_id,item_id,quantity,avg_unit_price,total_value) VALUES (?,?,?,?,?)",
                                (outward_id, row['item_id'], row['quantity'], avg_price, total_val))
                    if errors:
    ...
    with tab2:
        ...
        if df_out_all.empty:
            ...
            df_of = df_out_all[(df_out_all['indent_date'] >= pd.Timestamp(fr_dt)) &
                                (df_out_all['indent_date'] <= pd.Timestamp(to_dt2))]
            if srch:
            ...
            st.dataframe(df_of[['indent_no','indent_date','issued_to','location','section','remarks']],
                         use_container_width=True, hide_index=True)
            if not df_of.empty:
                sel_out_id = st.selectbox("Select entry for details",
                                           df_of['outward_id'].tolist(),
                                           format_func=lambda x: df_of.set_index('outward_id').loc[x,'indent_no'])
                conn = get_conn()
                det2 = pd.read_sql(
                    "SELECT d.*, i.item_code, i.item_name, i.unit FROM outward_details d "
                    "JOIN items i ON d.item_id=i.item_id WHERE d.outward_id=?",
                    conn, params=(sel_out_id,))
                conn.close()
                if not det2.empty:
    ...
    if not stock_df.empty:
        if not show_zero:
            stock_df = stock_df[stock_df['qty_in_stock'] > 0]
        if search_stk:
            stock_df = stock_df[stock_df.apply(lambda r: search_stk.lower() in str(r).lower(), axis=1)]
    if stock_df.empty:
        ...
        disp_cols = ['item_code','item_name','unit','location_code','location_name',
                     'category','qty_in_stock','avg_price_fmt','stock_value_fmt']
        st.dataframe(stock_df[disp_cols].rename(columns={
            'item_code':'Code','item_name':'Item Name','unit':'Unit',
            'location_code':'Loc Code','location_name':'Location',
            'category':'Category','qty_in_stock':'Qty in Stock',
        ...
        if item_opts2:
            ...
            if not batch_df.empty:
                ...
                st.info("No remaining FIFO batches for this item.")
    ...
    if report_type == "Stock Summary Report":
        ...
        df = pd.read_sql("""
            SELECT e.bill_no, e.bill_date, e.supplier_name,
                   i.item_code, it.item_name, it.unit,
            ...
            JOIN inward_details i ON e.inward_id=i.inward_id
            JOIN items it ON i.item_id=it.item_id
            WHERE e.bill_date BETWEEN ? AND ?
            ORDER BY e.bill_date, e.bill_no
        """, conn, params=(from_date.isoformat(), to_date.isoformat()))
        if df.empty:
            st.info("No inward records for selected period.")
        else:
        ...
        df = pd.read_sql("""
            SELECT e.indent_no, e.indent_date, e.issued_to, e.location, e.section,
                   ...
                   d.quantity, d.avg_unit_price, d.total_value
            FROM outward_entries e
            JOIN outward_details d ON e.outward_id=d.outward_id
            JOIN items it ON d.item_id=it.item_id
            WHERE e.indent_date BETWEEN ? AND ?
            ORDER BY e.indent_date, e.indent_no
        """, conn, params=(from_date.isoformat(), to_date.isoformat()))
        if df.empty:
            st.info("No issue records for selected period.")
        else:
        ...
        if item_opts3:
            ...
            inward_led = pd.read_sql("""
                SELECT e.bill_date AS txn_date, 'INWARD' AS txn_type,
                       e.bill_no AS ref_no, e.supplier_name AS party,
                       d.quantity AS qty_in, 0 AS qty_out,
            ...
            outward_led = pd.read_sql("""
                SELECT e.indent_date AS txn_date, 'ISSUE' AS txn_type,
                       e.indent_no AS ref_no, e.issued_to AS party,
                       0 AS qty_in, d.quantity AS qty_out,
            ...
            if ledger_df.empty:
                st.info("No transactions for this item in selected period.")
            else:
                ledger_df['balance'] = (ledger_df['qty_in'] - ledger_df['qty_out']).cumsum()
                st.dataframe(ledger_df, use_container_width=True, hide_index=True)
                tot_in  = ledger_df['qty_in'].sum()
                tot_out = ledger_df['qty_out'].sum()
                m1, m2, m3 = st.columns(3)
                m1.metric("Total Received", tot_in)
                m2.metric("Total Issued",   tot_out)
                m3.metric("Closing Balance", tot_in - tot_out)
        ...
        df = pd.read_sql("""
            SELECT e.section, e.location,
                   COUNT(DISTINCT e.outward_id) AS indents,
                   SUM(d.quantity) AS total_qty, SUM(d.total_value) AS total_value
            FROM outward_entries e JOIN outward_details d ON e.outward_id=d.outward_id
            WHERE e.indent_date BETWEEN ? AND ?
        ...
        """, conn, params=(from_date.isoformat(), to_date.isoformat()))
        if df.empty:
            st.info("No issue data for selected period.")
        else:
            df['total_value'] = df['total_value'].map(lambda x: f"₹{x:,.2f}")
            st.dataframe(df, use_container_width=True, hide_index=True)
